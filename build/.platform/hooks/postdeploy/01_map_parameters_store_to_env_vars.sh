...
# https://github.com/vahiwe/beanstalk-parameter-store-env-demo
...
echo ".platform/hooks/postdeploy/01_map_parameters_store_to_env_vars.sh executing"
...
if [ -z ${parameter_store_path+x} ]; then
  ...
  # Add the SSM parameters to the custom environment variable file
  while read -r line || [ -n "$line" ]; do
    echo "${line//"$parameter_store_path"/}" >> /opt/elasticbeanstalk/deployment/custom_env_var
